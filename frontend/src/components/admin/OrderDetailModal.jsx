...
import { fetchOrderById, clearSelectedOrder } from "../../store/admin/adminOrderSlice";
...
export default function OrderDetailModal({ orderId, onClose }) {
  ...
  const { selectedOrder, detailLoading, updating } = useSelector(s => s.adminOrders);
      ...
      <div className="bg-white w-full max-w-3xl rounded p-6">
        ...
        <div className="mt-2 border rounded">
          {selectedOrder.items?.map(it => (
            <div key={it.productSnapshot?._id} className="flex justify-between items-center border-b last:border-b-0 p-2">
        ...
        <div className="mt-4">
          ...
          <ul className="mt-2 divide-y">
            {selectedOrder.orderStatusHistory?.length ? (
              selectedOrder.orderStatusHistory.map((h, idx) => (
